...
int main(int argc, char **argv) //, char **envp)
{
    ...
    if (strlen(LOCAL_REPO) == 0) {
        s7x_repo = realpath("_main/repl", NULL);
    } else {
        s7x_repo = realpath("external/" LOCAL_REPO "/repl", NULL);
    }
    s7_add_to_load_path(s7, s7x_repo);

    char *script;
    // rules_cc rules define LOCAL_REPO for each src
    // file, on the cmd line. If bzlmod enabled, root repo is ""
    // (empty)? "the canonical repository name of the main repository
    // is always the empty string"
    log_debug("s7x LOCAL_REPO: '%s'", LOCAL_REPO);
    if (strlen(LOCAL_REPO) == 0) {
        script = "./repl/repl.scm";
    } else
        // bzlmod: how to deal with version number?
        // canonical name should include it?
        script = "external/" LOCAL_REPO "/repl/repl.scm";
    ...
    if (argc > 2) { //FIXME: use name param, e.g. -s, --script
        ...
        if (!s7_load(s7, script)) {
            log_error("failed: load %s; exiting", script);
            exit(EXIT_FAILURE);
        }
        s7_eval_c_string(s7, "((*repl* 'run))");
/* #else */
