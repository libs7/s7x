...
(define datafile "ast/dune")
(define m (call-with-input-file datafile dune:read))
...
(define datafile "ast/dune")
(define datafile "astlib/dune")
(define m (call-with-input-file datafile sexp:read))

(define tname "dune_to_toml.mustache")
(define t (call-with-input-file tname (lambda (p) (let ((tlen (length p))) (read-string tlen p)))))
t

m
(define ht (alist->hash-table m))

(define output (mustache:render #t t m 0))
output
